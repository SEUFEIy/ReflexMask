...
class InterpMaskedWideResNet(WideResNet):
    ...
    def _forward_conscious_ig(self, x, return_feats=False, external_masks=None):
        ...
        out = self.block1(out)
        if return_feats:
            activations['block1'] = out.clone()
        
        out = self.block2(out)
        out = apply_mask(out, 'block2')
        if return_feats:
            activations['block2'] = out.clone()
        
        out = self.block3(out)
        out = self.relu(self.bn1(out))
        out = F.avg_pool2d(out, 8)
        out_spatial = out.clone()  # Before flatten
        out = out.view(-1, self.nChannels)
        out = apply_mask(out, 'block3')
        if return_feats:
            activations['block3'] = out.clone()
            activations['block3_spatial'] = out_spatial

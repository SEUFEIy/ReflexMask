...
_SAVED_RANKINGS_DIR = _PROJECT_ROOT / "saved_rankings"
...
class InterpMaskedResNet(ResNet):
    def __init__(self, layer_name, checkpoint_name, mask_which, important_dim, block=BasicBlock, num_blocks=[2,2,2,2], num_classes=10, rs=False, rs_sigma=8/255, rs_nsmooth=1, batch_size=1000):
        ...
        if self.mask_which != 'none' and self.mask_which != 'twoforwardpasses' and self.mask_which != 'conscious_ig':
    ...
    def load_neuron_importance(self):
        if self.mask_which == 'loir':
            root_name = str(_SAVED_RANKINGS_DIR)
        ...
        if root_name is not None:
            folder_name = os.path.join(root_name, model_save_name, self.layer_name)

            ablated_acc = torch.zeros((self.layer_dim, self.num_classes)).cuda()

            # loading the neuron importance for every class
            print(f"  Loading {self.layer_dim} neuron importance files from {folder_name}/...")
            for k in tqdm(range(self.layer_dim), desc="  Loading rankings"):
                ablated_acc[k] = torch.Tensor(np.load(os.path.join(folder_name, f'unit{k}.npy')))
        else:
    ...
    def forward(self, x, layer=None, ablated_units=None, pred_cls=None, tau=1.0, defense=None, return_feats=False, external_masks=None):
        """
        Forward pass with optional defense mechanism
        
        Args:
            x: Input images
            layer: Layer name for masking (legacy)
            ablated_units: Units to ablate (legacy)
            pred_cls: Predicted class probabilities (legacy)
            tau: Temperature (legacy)
            defense: Defense mode - None (use self.mask_which), 'ig', 'conscious_ig', or 'none'
            return_feats: Whether to return layer activations
            external_masks: Dict of external masks {layer_name: mask} for conscious_ig
        
        Returns:
            If return_feats=False: logits
            If return_feats=True: (logits, dict of layer activations)
        """
        # Determine defense mode
        if defense is None:
            defense = self.mask_which
        
        # ReflexMask defense mode
        if defense == 'conscious_ig':
            return self._forward_conscious_ig(x, return_feats, external_masks)
        
        # Original ReflexMask modes
        if defense == 'twoforwardpasses':
    ...
    def _forward_conscious_ig(self, x, return_feats=False, external_masks=None):
        """
        Forward pass for conscious_ig defense with optional external masks
        
        Args:
            x: Input images
            return_feats: Whether to return layer activations
            external_masks: Dict {layer_name: mask tensor [batch, channels] or [channels]}
        ...
        def apply_mask(out, layer_name):
            """Apply external mask if provided"""
            if external_masks is not None and layer_name in external_masks:
                mask = external_masks[layer_name]
                # Handle different mask shapes
                if mask.dim() == 1:
                    # [channels] -> [1, channels, 1, 1] for spatial features
                    if out.dim() == 4:
        ...
        out = self.layer1(out)
        out = apply_mask(out, 'layer1')
        if return_feats:
            activations['layer1'] = out.clone()
        
        out = self.layer2(out)
        out = apply_mask(out, 'layer2')
        if return_feats:
            activations['layer2'] = out.clone()
        
        out = self.layer3(out)
        out = apply_mask(out, 'layer3')
        if return_feats:
            activations['layer3'] = out.clone()
        
        out = self.layer4(out)
        out = F.avg_pool2d(out, 4)
        out_spatial = out.clone()  # Before flatten, for visualization
        out = out.view(out.size(0), -1)
        out = apply_mask(out, 'layer4')
        if return_feats:
            activations['layer4'] = out.clone()
            activations['layer4_spatial'] = out_spatial

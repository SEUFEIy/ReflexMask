...
_SAVED_LOGITS_DIR = _PROJECT_ROOT / "saved_predicted_logits"
_SAVED_LOGITS_DIR.mkdir(exist_ok=True)
...
def get_loaders(dataset, dir_, batch_size, arch, mode='Crop288', num_workers=0):
    if dataset == 'cifar10':
        if arch in ['rn18', 'rn18_val', 'wrn34_10', 'rn50', 'preactresnet18']:
        ...
        if arch in ['rn18', 'rn18_val', 'parn18_oaat', 'wrn34_10_las', 'wrn34_10', 'rn50', 'preactresnet18']:
    ...
    if dataset == 'cifar10':
        ...
        # Use project-level ImageNet data path
        imagenet_dir = os.path.join(dir_, 'imagenet')
        train_eval_dataset = datasets.ImageFolder(os.path.join(imagenet_dir, 'val'), test_transform)
        test_dataset = datasets.ImageFolder(os.path.join(imagenet_dir, 'val'), test_transform)
...
def save_predicted_logits(test_loader, model, ckpt_name, num_classes=10):
    ...
    os.makedirs(_SAVED_LOGITS_DIR, exist_ok=True)
    torch.save(output_logits, str(_SAVED_LOGITS_DIR / f'{ckpt_name[:-4]}_predlogits.pth'))
...
def compare_predicted_probs(test_loader, model, ckpt_name, layer_name='layer4', ablated_units=None, num_classes=10, use_probs=False, gt_only=False):
    orig_output_logits = torch.load(str(_SAVED_LOGITS_DIR / f'{ckpt_name[:-4]}_predlogits.pth'))
